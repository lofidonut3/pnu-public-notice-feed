...
class WebSquareClient {
  constructor({ baseUrl, sitePath, menuCd, cookie, csrfToken, rsaKey }) {
  ...
  static async create({ baseUrl, sitePath, menuCd }) {
    ...
    if (!response.ok) throw new Error(`failed to fetch websquare entry: ${response.status}`);
  ...
  async postJson(path, payload, authStr = {}) {
    ...
    if (!response.ok) {
      throw new Error(`websquare ${path} failed: ${response.status} ${text.slice(0, 300)}`);
    }
    try {
      return JSON.parse(text);
    } catch {
      throw new Error(`websquare ${path} returned non-json: ${text.slice(0, 300)}`);
    }
  }
}
...
function normalizeAttachments(fileList, row, baseUrl) {
  if (Array.isArray(fileList) && fileList.length > 0) {
    return fileList
      .filter((file) => file?.PARAM_CODE && file?.ORIGIN_FILE_NM)
      .map((file) => ({
        name: String(file.ORIGIN_FILE_NM),
        url: `${baseUrl}/file/download.do?${file.PARAM_CODE}`,
    ...
    return [
      {
        ...
        url: `${baseUrl}/file/download.do?${row.PARAM_CODE}`,
